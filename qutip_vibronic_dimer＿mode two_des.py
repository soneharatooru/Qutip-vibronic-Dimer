...
# "Toy 2DES" utilities
# ----------------------
def two_des_waiting_time_signal(
    deps: float = 120.0,
    J: float = 100.0,
    ws: float = 200.0,
    wa: float = 210.0,
    ...
    """
    Produce a simplified (surrogate) 2DES waiting-time trace S(T) and spectrum.
    This is NOT a full third-order response. It approximates the rephasing beat
    amplitude by tracking the exciton coherence magnitude |rho_{+,-}(T)| after
    an impulsive preparation (equal exciton superposition) and free evolution
    under Lindblad dynamics. It is useful to visualize vibronic beat frequencies.

    Returns:
        Tlist (fs), S(T) (arb.), freq_cm (cm^-1), spectrum (arb.)
    """
    # Reuse the simulate() but only need one run up to Tmax_T
    Tlist, coh, Rvals = simulate(
        deps=deps, J=J, ws=ws, wa=wa, gs=gs, ga=ga,
        gamma_s=gamma_s, gamma_a=gamma_a, gamma_phi=gamma_phi,
        Nphonon=Nphonon, Tmax=Tmax_T, dt=dT, T_kelvin=T_kelvin
    )

    # Window and zero-mean for cleaner FFT
    y = coh - np.mean(coh)
    window = np.hanning(len(y))
    y_win = y * window

    # FFT -> convert to wavenumbers (cm^-1)
    dt_fs = dT
    freqs_fs = np.fft.rfftfreq(len(y_win), d=dt_fs)  # in 1/fs
    spec = np.abs(np.fft.rfft(y_win))
    # convert linear frequency (1/fs) to cm^-1: ν(cm^-1) = f(1/fs) / c_cmfs
    freq_cm = freqs_fs / c_cmfs

    return Tlist, coh, freq_cm, spec

def plot_two_des(Tlist, S_T, freq_cm, spec, out_prefix="two_des"):
    # Waiting-time trace
    plt.figure()
    plt.plot(Tlist, S_T, label="S(T) ~ |rho_{+,-}(T)|")
    plt.xlabel("Waiting time T (fs)"); plt.ylabel("Beat amplitude (arb.)")
    plt.title("Toy 2DES waiting-time trace")
    plt.legend(); plt.tight_layout()
    plt.savefig(f"{out_prefix}_wait_trace.png", dpi=150)

    # Spectrum
    plt.figure()
    plt.plot(freq_cm, spec, label="FFT[S(T)]")
    plt.xlabel("Frequency (cm$^{-1}$)"); plt.ylabel("Amplitude (arb.)")
    plt.title("Toy 2DES waiting-time spectrum")
    plt.xlim(0, max(1200, freq_cm[-1]))
    plt.legend(); plt.tight_layout()
    plt.savefig(f"{out_prefix}_wait_spectrum.png", dpi=150)
...
def main():
    ...
    p.add_argument("--mode", choices=["single_run", "sweep_gamma", "two_des"], default="single_run")
    ...
    elif args.mode == "two_des":
        # Use the same parameters but generate a simplified 2DES waiting-time trace and its spectrum
        Tlist, S_T, freq_cm, spec = two_des_waiting_time_signal(
            deps=args.deps, J=args.J, ws=args.ws, wa=args.wa,
            gs=args.gs, ga=args.ga,
            gamma_s=args.gamma_s, gamma_a=args.gamma_a, gamma_phi=args.gamma_phi,
            Nphonon=args.Nphonon, Tmax_T=args.Tmax, dT=args.dt, T_kelvin=args.T
        )
        plot_two_des(Tlist, S_T, freq_cm, spec, out_prefix=args.out)
        print("Done. Saved:", f"{args.out}_wait_trace.png", f"{args.out}_wait_spectrum.png")
